 ...
 *  \author Petyovský
 *  \version 2024
 *  $Id: TVector.h 2670 2024-03-25 14:21:39Z petyovsky $
 */
 ...
 *  \todo Dokončit pro semestr 2024 (rozdělit na dvě API: vector_iterators_are_same() a vector_iterators_are_equal(), promyslet rozdíly v jejich chování)
 *  \param[in] aLeftIter Levý iterátor
 *  \param[in] aRightIter Pravý iterátor
 *  \return \c true pokud iterátory odkazují na element na stejné pozici a ve stejném vektoru případně jsou oba neplatné
 */
bool vector_iterators_are_same(const struct TVectorIterator *aLeftIter, const struct TVectorIterator *aRightIter);

// *\details Vzájemně porovná, zda oba iterátory směřují po výpočtu adresy na totéž místo v paměti. (nikoliv, že mají shodné položky)
// *\return \c true pokud iterátory odkazují na element na stejné pozici a oba iterátory jsou platné
// \todo Dokoncit pro semestr 2024
//bool vector_iterators_are_equal(const struct TVectorIterator *aLeftIter, const struct TVectorIterator *aRightIter);
